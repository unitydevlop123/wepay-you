...
$package_data = readJsonFile('legend');

// Debug: Log what we get from Firebase
if ($package_data) {
    error_log("LEGEND DEBUG: Firebase data loaded, image: " . ($package_data['image'] ?? 'no image'));
} else {
    error_log("LEGEND DEBUG: No Firebase data, using fallback");
}
...
if ($package_data) {
    ...
    $package_data['image'] = 'attached_assets/attached_assets/generated_images/Wealth_management_executive_ccd05c9a.png';
    error_log("LEGEND DEBUG: Overriding image path to: " . $package_data['image']);
} else {
    ...
    $package_data = [
        ...
        'description' => 'Premium Legend package with excellent returns.',
        'image' => 'attached_assets/attached_assets/generated_images/Wealth_management_executive_ccd05c9a.png',
        'id' => 11,
        'popular' => true
    ];
}
...
$current_user_email = $_SESSION['email'] ?? null; // Store current user's email
        ...
        foreach ($users as $user) {
            if ($user['email'] === $_SESSION['email']) {
                ...
                foreach ($active_packages as $active_package) {
                    // The package_data['name'] is specific to the current page being viewed.
                    // For example, if viewing bronze.php, package_data['name'] will be "Bronze".
                    // We need to ensure this matches the package name stored in active_packages.
...
if ($_POST['action'] ?? '') {
    if ($_POST['action'] === 'purchase' && $can_afford && !$already_purchased) {
        // Ensure we have user data and package data to proceed
        if (isset($user_data) && isset($package_data) && $current_user_email) {
            $users = getUsers();
...
<body>
    <div class="main-container">
        ...
        <div class="banner-section">
            <img src="<?= htmlspecialchars($package_data['image'] ?? 'default_image.jpg') ?>" alt="<?= htmlspecialchars($package_data['name'] ?? 'Package') ?> Package" class="banner-image">
