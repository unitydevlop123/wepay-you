...
<head>
    ...
    <title>LET PAY YOU - Crash Rocket</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        @keyframes rocket-launch {
            ...
            100% { transform: translateY(-200px) rotate(-30deg) scale(1.5); opacity: 0; }
        }
...
<body>
    <div class="main-container">
        ...
        <div class="game-header">
            <img src="attached_assets/attached_assets/generated_images/Friends_celebrating_gaming_win_ad648a5b.png" alt="Crash Background" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; object-fit: cover; z-index: 0;">
            <div class="header-content">
                <h1 class="game-title">🚀 Crash Rocket</h1>
                <p class="game-subtitle">Rocket to the moon! Cash out before the crash!</p>
        ...
        <div class="game-content">
            ...
            <div class="recent-results">
                <div class="results-header">🚀 Recent Crashes</div>
                <div class="results-list" id="results-list">
                    <div class="result-item medium">3.45x</div>
                    <div class="result-item low">1.02x</div>
                    <div class="result-item high">7.89x</div>
                    <div class="result-item medium">2.67x</div>
                    <div class="result-item low">1.15x</div>
                </div>
            </div>
            ...
            <div class="crash-game">
                <div class="game-screen">
                    ...
                    <div class="game-status" id="game-status">Get ready for takeoff! Place your bet!</div>
                ...
                <div class="betting-panel">
                    ...
                    <button class="place-bet-btn" id="place-bet-btn" onclick="placeBet()">
                        Launch Rocket - ₦<span id="bet-display">100</span>
                    </button>
                </div>
            </div>
        </div>
    ...
    <!-- Include Balance Connector System -->
    <script>
        // Set global balance variables for connector
        window.userBalance = <?= $user_balance_main ?>;
        window.userEmail = '<?= $_SESSION["email"] ?>';
    </script>
    <script src="js/games-balance-connector.js"></script>
    ...
    <script>
        ...
        async function placeBet() {
            ...
            // Check against the maximum bet limit defined in PHP
            const maxBetLimit = 5000000; // This should ideally be fetched from the server or a config
            if (betAmount > maxBetLimit) {
                alert(`Maximum bet is ₦${maxBetLimit.toLocaleString()}`);
                return;
            }

            if (gameState.isPlaying) {
                alert('Rocket in flight! Wait for current round to finish.');
                return;
            }

            // Deduct bet amount from user balance
            try {
                const response = await fetch('crash.php', {
                    method: 'POST',
                    headers: {
                ...
                // Success - bet was deducted
                console.log('✅ Crash bet placed successfully!');
                ...
                document.getElementById('place-bet-btn').textContent = 'Rocket Launching...';

                // Start countdown
                let countdown = 3;
                document.getElementById('game-status').textContent = `Launching in ${countdown}...`;
                ...
                console.log('❌ Crash bet failed:', error);
        ...
        function startGame() {
            ...
            document.getElementById('game-status').textContent = '🚀 Rocket is flying! Cash out before it crashes!';
            document.getElementById('cash-out-btn').classList.add('active');
            document.getElementById('rocket').style.animation = 'rocket-launch 10s linear infinite';

            // Game loop
            gameState.gameInterval = setInterval(() => {
                // Increase multiplier (faster increase as it goes higher)
                const baseIncrease = 0.01;
                const acceleratedIncrease = Math.pow(gameState.currentMultiplier - 1, 1.1) * 0.005;
                gameState.currentMultiplier += baseIncrease + acceleratedIncrease;
                ...
                const crashChance = Math.min(0.01 + Math.pow(gameState.currentMultiplier - 1, 1.5) * 0.008, 0.15);
        ...
        async function cashOut() {
            ...
            try {
                ...
                // Success - winnings added
                console.log('🎉 Crash winnings added successfully!');
                ...
                document.getElementById('rocket').style.animation = 'rocket-idle 2s ease-in-out infinite';
                ...
                // Error adding winnings
                console.log('❌ Adding Crash winnings failed:', error);
        ...
        function crashGame() {
            ...
            document.getElementById('game-status').innerHTML = 
                `💥 Rocket crashed at ${gameState.currentMultiplier.toFixed(2)}x!<br>Better luck next flight!`;
            document.getElementById('cash-out-btn').classList.remove('active');
        ...
        function resetGame() {
            ...
            document.getElementById('game-status').textContent = 'Get ready for takeoff! Place your bet!';
            document.getElementById('place-bet-btn').disabled = false;
            document.getElementById('place-bet-btn').innerHTML = 'Launch Rocket - ₦<span id="bet-display">' + gameState.betAmount + '</span>';
            document.getElementById('rocket').textContent = '🚀';
            document.getElementById('rocket').style.animation = 'rocket-idle 2s ease-in-out infinite';
        }
