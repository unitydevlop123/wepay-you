...
$package_data = readJsonFile('sapphire');

// Debug: Log what we get from Firebase
if ($package_data) {
    error_log("SAPPHIRE DEBUG: Firebase data loaded, image: " . ($package_data['image'] ?? 'no image'));
} else {
    error_log("SAPPHIRE DEBUG: No Firebase data, using fallback");
}
...
if ($package_data) {
    ...
    $package_data['image'] = 'attached_assets/attached_assets/generated_images/Investment_team_collaboration_86961e9e.png';
    error_log("SAPPHIRE DEBUG: Overriding image path to: " . $package_data['image']);
} else {
    ...
    $package_data = [
        ...
        'total_profit' => 75000,
        'total_return' => 100000,
        'description' => 'Premium Sapphire package with excellent returns.',
        'image' => 'attached_assets/attached_assets/generated_images/Investment_team_collaboration_86961e9e.png',
        'id' => 7,
        'popular' => false
    ];
}
    ...
    if (!empty($users)) {
        foreach ($users as $user) {
            if ($user['email'] === $_SESSION['email']) {
                // Assign user data to a variable for easier access
                $user_data = $user;
                ...
                // Fallback: Check old single package system
                if (!$already_purchased && isset($user_data['sapphire_package']) && $user_data['sapphire_package']['active'] === true) {
                    $already_purchased = true;
                    $package_expiry = $user_data['sapphire_package']['expiry_date'] ?? 'Unknown';
                }
                break;
            }
        }
...
if (!empty($_POST['action'])) {
    if ($_POST['action'] === 'purchase' && $can_afford) {
        $users = getUsers();
        $current_user_email = $_SESSION['email']; // Store current user email
        for ($i = 0; $i < count($users); $i++) {
            if ($users[$i]['email'] === $_SESSION['email']) {
                ...
                // Check if active_packages array exists, if not create it
                if (!isset($users[$i]['active_packages']) || !is_array($users[$i]['active_packages'])) {
                    $users[$i]['active_packages'] = [];
                }

                // Remove old single package references if they exist
                unset($users[$i]['active_package']);
                unset($users[$i]['package_id']);

                // Add new package with proper structure for orders system
        ...
        $notification = [
            ...
            'message' => "🎉 You've successfully purchased the {$package['name']} package for ₦" . number_format($package['capital'], 2) . ". Start completing orders to earn daily profits!",
            'amount' => 0,
            'status' => 'unread',
