...
$package_data = readJsonFile('ruby');

// Debug: Log what we get from Firebase
if ($package_data) {
    error_log("RUBY DEBUG: Firebase data loaded, image: " . ($package_data['image'] ?? 'no image'));
} else {
    error_log("RUBY DEBUG: No Firebase data, using fallback");
}
...
if ($package_data) {
    ...
    $package_data['image'] = 'attached_assets/attached_assets/generated_images/Business_presentation_scene_44488820.png';
    error_log("RUBY DEBUG: Overriding image path to: " . $package_data['image']);
} else {
    ...
    $package_data = [
        ...
        'total_profit' => 37500,
        'total_return' => 52500,
        'description' => 'Premium Ruby package with excellent returns.',
        'image' => 'attached_assets/attached_assets/generated_images/Business_presentation_scene_44488820.png',
        'id' => 6,
        'popular' => false
    ];
}
...
// Placeholder for user data to be populated
$user_data = [];
$current_user_email = $_SESSION['email'] ?? null; // Use null coalescing operator for safety
    ...
    if (!empty($users)) {
        foreach ($users as $user) {
            if ($user['email'] === $current_user_email) {
                // Populate user_data with the current user's information
                $user_data = $user;
                ...
                if (isset($user_data['active_packages']) && !empty($user_data['active_packages'])) {
                    foreach ($user_data['active_packages'] as $active_package) {
                        // Comparing package names directly, ensure they match 'ruby.json' name from package_data
                        if ($active_package['name'] === $package_data['name']) {
                            ...
                            if ($expiry_date > $today) {
                                ...
                                break; // Found an active package, no need to check further for this type
                            }
                        }
                    }
                }
...
// Determine if the user can afford the package and hasn't purchased it yet
$can_afford = $user_balance >= ($package_data['capital'] ?? 0) && !$already_purchased;

if (!empty($_POST['action'])) {
    if ($_POST['action'] === 'purchase' && $can_afford && !$already_purchased && $current_user_email) {
        // Ensure we are working with the correct user data fetched earlier
        if (!empty($user_data)) {
            $users = getUsers();
            for ($i = 0; $i < count($users); $i++) {
                if ($users[$i]['email'] === $current_user_email) {
                    ...
                    // Remove old single package entry if it exists, to avoid conflict and ensure data consistency
                    if (isset($users[$i]['ruby_package'])) {
                        unset($users[$i]['ruby_package']);
                    }
            ...
            // Redirect on success. The original code redirects to dashboard.php.
...
<head>
    ...
    <title><?= htmlspecialchars($package_data['name'] ?? 'Package') ?> Package - LET PAY YOU</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="main-container">
        ...
        <div class="banner-section">
            <img src="<?= htmlspecialchars($package_data['image'] ?? 'default_banner.jpg') ?>" alt="<?= htmlspecialchars($package_data['name'] ?? 'Package') ?> Package" class="banner-image">
        ...
        <div class="content">
            ...
            <div class="package-details">
                ...
                <div class="details-grid">
                    ...
                    <div class="detail-item">
                        ...
                        <div class="detail-value"><?= htmlspecialchars($package_data['duration'] ?? 0) ?> Days</div>
                    </div>
                    <div class="detail-item">
                        <div class="detail-label">Daily Orders</div>
                        <div class="detail-value"><?= htmlspecialchars($package_data['orders'] ?? 0) ?></div>
                    </div>
                    <div class="detail-item">
            ...
            <div class="info-section">
                ...
                <div class="info-grid">
                    <div class="info-item">
                        <h4>Daily Orders</h4>
                        <p>Complete <?= htmlspecialchars($package_data['orders'] ?? 0) ?> simple orders daily to earn <?= formatAmount($package_data['daily_profit'] ?? 0) ?></p>
                    </div>
                    <div class="info-item">
                        <h4>Work Time</h4>
                    ...
                    <div class="info-item">
                        <h4>Payment</h4>
                        <p>Earn daily profits automatically. Consistency is key for <?= htmlspecialchars($package_data['duration'] ?? 0) ?> days</p>
                    </div>
                </div>
            </div>
