...
// Assuming firebase_setup.php sets up necessary Firebase SDK and functions like getUsers, saveUsers, readJsonFile, writeJsonFile, and addUserNotification

// Load package data from Firebase
$package_data = readJsonFile('bronze');

// Debug: Log what we get from Firebase
if ($package_data) {
    error_log("BRONZE DEBUG: Firebase data loaded, image: " . ($package_data['image'] ?? 'no image'));
} else {
    error_log("BRONZE DEBUG: No Firebase data, using fallback");
}
...
if ($package_data) {
    ...
    $package_data['image'] = 'attached_assets/attached_assets/generated_images/Investment_advisor_portrait_46777176.png';
    error_log("BRONZE DEBUG: Overriding image path to: " . $package_data['image']);
} else {
    ...
    $package_data = [
        ...
        'total_profit' => 1500,
        'total_return' => 3000,
        'description' => 'Premium Bronze package with excellent returns.',
        'image' => 'attached_assets/attached_assets/generated_images/Investment_advisor_portrait_46777176.png',
        'id' => 1,
        'popular' => false
    ];
}
...
if (isset($_SESSION['email'])) {
    $users = getUsers(); // Assume this function reads user data from Firebase or a JSON file
    $current_user_email = $_SESSION['email']; // Store current user email for Firebase functions

    if (!empty($users)) {
        foreach ($users as $user) {
            if ($user['email'] === $_SESSION['email']) {
                // Assign user data to a more generic variable for easier access
                $user_data = $user;
                $user_balance = (float)($user_data['balance'] ?? 0.00);

                // Check active packages for this package type
                $active_packages = $user_data['active_packages'] ?? [];
                foreach ($active_packages as $active_package) {
                    // Ensure we are comparing against the correct package name from $package_data
                    if (isset($package_data['name']) && $active_package['name'] === $package_data['name']) {
...
$already_purchased = false; // Resetting here to ensure logic below is sound
$package_expiry = '';
$can_afford = $user_balance >= ($package_data['capital'] ?? 0); // Use $user_balance here

// Check new multi-package system first
// Use $user_data which is populated from the user's session
if (isset($user_data['active_packages']) && !empty($user_data['active_packages'])) {
    foreach ($user_data['active_packages'] as $package) {
        if ($package['name'] === $package_data['name']) { // Use $package_data['name'] for dynamic checking
...
// Fallback: Check old single package system if not already found in the new system
// This part might need adjustment based on how old packages were stored.
// Assuming 'bronze_package' was a specific key in the old system.
if (!$already_purchased && isset($user_data['bronze_package']) && $user_data['bronze_package']['active'] === true) {
    $already_purchased = true;
    $package_expiry = $user_data['bronze_package']['expiry_date'] ?? 'Unknown';
}


function formatAmount($amount) {
    // Assuming currency is Naira (₦)
    return '₦' . number_format($amount, 2);
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') { // Use $_SERVER['REQUEST_METHOD'] for better practice
    if (($_POST['action'] ?? '') === 'purchase' && $can_afford && !$already_purchased) {
        $users = getUsers(); // Re-fetch users to ensure latest data
        $sanitized_email = filter_var($_SESSION['email'], FILTER_SANITIZE_EMAIL); // Sanitize email
        ...
        for ($i = 0; $i < count($users); $i++) {
            if ($users[$i]['email'] === $_SESSION['email']) {
                ...
                // Initialize active_packages if not exists
                if (!isset($users[$i]['active_packages'])) {
                    $users[$i]['active_packages'] = [];
                }

                // Add new package to active packages
                $expiry_date = date('Y-m-d', strtotime('+12 days')); // 12-day duration, adjust as needed
                $users[$i]['active_packages'][] = [
                ...
                saveUsers($users); // Assume this function saves user data back to Firebase or JSON file
...
<body>
    <div class="main-container">
        ...
        <div class="banner-section">
            <img src="<?= htmlspecialchars($package_data['image']) ?>" alt="<?= htmlspecialchars($package_data['name']) ?> Package" class="banner-image">
        ...
        <div class="content">
            ...
            <div class="package-details">
                ...
                <div class="details-grid">
                    ...
                    <div class="detail-item">
                        ...
                        <div class="detail-value"><?= htmlspecialchars($package_data['duration']) ?> Days</div>
                    </div>
                    <div class="detail-item">
                        <div class="detail-label">Daily Orders</div>
                        <div class="detail-value"><?= htmlspecialchars($package_data['orders']) ?></div>
                    </div>
                    <div class="detail-item">
            ...
            <div class="info-section">
                ...
                <div class="info-grid">
                    <div class="info-item">
                        <h4>Daily Orders</h4>
                        <p>Complete <?= htmlspecialchars($package_data['orders']) ?> simple orders daily to earn <?= formatAmount($package_data['daily_profit']) ?></p>
                    </div>
                    <div class="info-item">
                        <h4>Work Time</h4>
                    ...
                    <div class="info-item">
                        <h4>Payment</h4>
                        <p>Earn daily profits automatically. Consistency is key for <?= htmlspecialchars($package_data['duration']) ?> days</p>
                    </div>
                </div>
            </div>
