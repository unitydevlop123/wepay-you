...
$package_data = readJsonFile('silver');

// Debug: Log what we get from Firebase
if ($package_data) {
    error_log("SILVER DEBUG: Firebase data loaded, image: " . ($package_data['image'] ?? 'no image'));
} else {
    error_log("SILVER DEBUG: No Firebase data, using fallback");
}
...
if ($package_data) {
    ...
    $package_data['image'] = 'attached_assets/attached_assets/generated_images/Luxury_investment_advisor_46491d9b.png';
    error_log("SILVER DEBUG: Overriding image path to: " . $package_data['image']);
} else {
    ...
    $package_data = [
        ...
        'total_profit' => 3000,
        'total_return' => 5500,
        'description' => 'Premium Silver package with excellent returns.',
        'image' => 'attached_assets/attached_assets/generated_images/Luxury_investment_advisor_46491d9b.png',
        'id' => 2,
        'popular' => false
    ];
}
...
$package_expiry = null;
$current_user_email = $_SESSION['email'] ?? null; // Store current user email
                    ...
                    foreach ($user_data['active_packages'] as $active_package) {
                        if ($active_package['name'] === $package_data['name']) {
                            ...
                            if ($expiry_date > $today) {
                                ...
                                break; // Found an active package, no need to check further
                            }
                        }
                    }
                }

                // Fallback: Check old single package system if not already purchased
                if (!$already_purchased && isset($user_data['silver_package']) && $user_data['silver_package']['active'] === true) {
                    $already_purchased = true;
                    $package_expiry = $user_data['silver_package']['expiry_date'] ?? 'Unknown';
                }
                break;
            }
        }
