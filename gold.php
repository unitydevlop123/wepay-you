...
$package_data = readJsonFile('gold');

// Debug: Log what we get from Firebase
if ($package_data) {
    error_log("GOLD DEBUG: Firebase data loaded, image: " . ($package_data['image'] ?? 'no image'));
} else {
    error_log("GOLD DEBUG: No Firebase data, using fallback");
}
...
if ($package_data) {
    ...
    $package_data['image'] = 'attached_assets/attached_assets/generated_images/Professional_investment_advisor_headshot_9cd4e93e.png';
    error_log("GOLD DEBUG: Overriding image path to: " . $package_data['image']);
} else {
    ...
    $package_data = [
        ...
        'total_profit' => 3600,
        'total_return' => 6600,
        'description' => 'Premium Gold package',
        'image' => 'attached_assets/attached_assets/generated_images/Professional_investment_advisor_headshot_9cd4e93e.png',
        'id' => 3
    ];
}
    ...
    if (!empty($users)) {
        foreach ($users as $user) {
            if ($user['email'] === $_SESSION['email']) {
                $user_data = $user; // Assign the current user's data to $user_data
                $user_balance = (float)($user['balance'] ?? 0.00);

                // Check if user already has this package
                $already_purchased = false;
                $package_expiry = '';
                $can_afford = $user_balance >= ($package_data['capital'] ?? 0);

                // Check new multi-package system first
                if (isset($user_data['active_packages']) && !empty($user_data['active_packages'])) {
                    foreach ($user_data['active_packages'] as $active_package) {
                ...
                // Fallback: Check old single package system if the package is not found in the new system
                if (!$already_purchased && isset($user_data['gold_package']) && $user_data['gold_package']['active'] === true) {
                    $already_purchased = true;
                    $package_expiry = $user_data['gold_package']['expiry_date'] ?? 'Unknown';
                }
...
if (!empty($_POST['action'])) {
    if ($_POST['action'] === 'purchase' && $can_afford && !$already_purchased) {
        $users = getUsers();
        $current_user_email = $_SESSION['email']; // Get current user email for notification
        ...
        for ($i = 0; $i < count($users); $i++) {
            if ($users[$i]['email'] === $current_user_email) {
                ...
                $duration = $package_data['duration'] ?? $package_data['duration_days'] ?? 30;
    ...
    <style>
        * {
        ...
        .confirm-btn {
            background: #10b981;
            color: white;
            padding: 10px 30px; /* Made button smaller */
            border: none;
            border-radius: 30px; /* Adjusted border-radius to match smaller size */
        ...
        .confirm-btn:hover {
            ...
            box-shadow: 0 5px 15px rgba(16, 185, 129, 0.4); /* Added hover effect */
        }
...
<body>
    <div class="main-container">
        ...
        <div class="banner-section">
            <img src="<?= htmlspecialchars($package_data['image'] ?? 'default_banner.jpg') ?>" alt="<?= htmlspecialchars($package_data['name']) ?> Package" class="banner-image">
        ...
        <div class="content">
            ...
            <div class="package-details">
                ...
                <div class="details-grid">
                    <div class="detail-item">
                        ...
                        <div class="detail-value highlight-value"><?= formatAmount($package_data['capital'] ?? 0) ?></div>
                    </div>
                    <div class="detail-item">
                        <div class="detail-label">Daily Profit</div>
                        <div class="detail-value highlight-value"><?= formatAmount($package_data['daily_profit'] ?? 0) ?></div>
                    </div>
                    <div class="detail-item">
                    ...
                    <div class="detail-item">
                        ...
                        <div class="detail-value highlight-value"><?= formatAmount($package_data['total_profit'] ?? 0) ?></div>
                    </div>
                    <div class="detail-item">
                        <div class="detail-label">Total Return</div>
                        <div class="detail-value highlight-value"><?= formatAmount($package_data['total_return'] ?? 0) ?></div>
                    </div>
                </div>
            </div>
            ...
            <div class="info-section">
                ...
                <div class="info-grid">
                    <div class="info-item">
                        <h4>Daily Orders</h4>
                        <p>Complete <?= htmlspecialchars($package_data['orders'] ?? 'N/A') ?> simple orders daily to earn <?= formatAmount($package_data['daily_profit'] ?? 0) ?></p>
                    </div>
                    <div class="info-item">
                        <h4>Work Time</h4>
        ...
        function confirmInvestment() {
            ...
            if (confirm('Are you sure you want to invest in the <?= htmlspecialchars($package_data['name']) ?> package for <?= formatAmount($package_data['capital'] ?? 0) ?>?\n\nThis will be deducted from your wallet balance.')) {
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            ...
                            const packageCost = <?= $package_data['capital'] ?? 0 ?>;
