...
$package_data = readJsonFile('platinum');

// Debug: Log what we get from Firebase
if ($package_data) {
    error_log("PLATINUM DEBUG: Firebase data loaded, image: " . ($package_data['image'] ?? 'no image'));
} else {
    error_log("PLATINUM DEBUG: No Firebase data, using fallback");
}
...
if ($package_data) {
    ...
    $package_data['image'] = 'attached_assets/attached_assets/generated_images/Distinguished_investment_banker_executive_7f8f3e33.png';
    error_log("PLATINUM DEBUG: Overriding image path to: " . $package_data['image']);
} else {
    ...
    $package_data = [
        ...
        'total_profit' => 7500,
        'total_return' => 12500,
        'description' => 'Premium Platinum package with excellent returns.',
        'image' => 'attached_assets/attached_assets/generated_images/Distinguished_investment_banker_executive_7f8f3e33.png',
        'id' => 4,
        'popular' => false
    ];
}
...
// Assuming 'platinum_package' was a specific key in the old system.
if (!$already_purchased && isset($user_data['platinum_package']) && $user_data['platinum_package']['active'] === true) {
    $already_purchased = true;
    $package_expiry = $user_data['platinum_package']['expiry_date'] ?? 'Unknown';
}
    ...
    if (($_POST['action'] ?? '') === 'purchase' && $can_afford && !$already_purchased) {
        ...
        for ($i = 0; $i < count($users); $i++) {
            if ($users[$i]['email'] === $_SESSION['email']) {
                ...
                $expiry_date = date('Y-m-d', strtotime('+' . $package_data['duration'] . ' days')); // Use package duration
                $users[$i]['active_packages'][] = [
