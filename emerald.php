...
$package_data = readJsonFile('emerald');

// Debug: Log what we get from Firebase
if ($package_data) {
    error_log("EMERALD DEBUG: Firebase data loaded, image: " . ($package_data['image'] ?? 'no image'));
} else {
    error_log("EMERALD DEBUG: No Firebase data, using fallback");
}
...
if ($package_data) {
    ...
    $package_data['image'] = 'attached_assets/attached_assets/generated_images/Elite_wealth_manager_portrait_03302be9.png';
    error_log("EMERALD DEBUG: Overriding image path to: " . $package_data['image']);
} else {
    ...
    $package_data = [
        ...
        'total_profit' => 175000,
        'total_return' => 225000,
        'description' => 'Premium Emerald package with excellent returns.',
        'image' => 'attached_assets/attached_assets/generated_images/Elite_wealth_manager_portrait_03302be9.png',
        'id' => 8,
        'popular' => false
    ];
}
...
// Placeholder for user data to be populated later
$user_data = [];
    ...
    if (!empty($users)) {
        foreach ($users as $user) {
            if ($user['email'] === $_SESSION['email']) {
                $user_data = $user; // Store the current user's data
                ...
                if (!$already_purchased && isset($user_data['emerald_package']) && $user_data['emerald_package']['active'] === true) {
                    $already_purchased = true;
                    $package_expiry = $user_data['emerald_package']['expiry_date'] ?? 'Unknown';
                }
                break;
            }
        }
    ...
    if ($_POST['action'] === 'purchase' && $can_afford && !$already_purchased) {
        ...
        for ($i = 0; $i < count($users); $i++) {
            if ($users[$i]['email'] === $_SESSION['email']) {
                ...
                // Remove from old single package system if it exists
                if (isset($users[$i]['emerald_package'])) {
                    unset($users[$i]['emerald_package']);
                }
