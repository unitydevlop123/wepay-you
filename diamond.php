...
$package_data = readJsonFile('diamond');

// Debug: Log what we get from Firebase
if ($package_data) {
    error_log("DIAMOND DEBUG: Firebase data loaded, image: " . ($package_data['image'] ?? 'no image'));
} else {
    error_log("DIAMOND DEBUG: No Firebase data, using fallback");
}
...
if ($package_data) {
    ...
    $package_data['image'] = 'attached_assets/attached_assets/generated_images/Elite_female_financial_consultant_ff395274.png';
    error_log("DIAMOND DEBUG: Overriding image path to: " . $package_data['image']);
} else {
    ...
    $package_data = [
        ...
        'image' => 'attached_assets/attached_assets/generated_images/Elite_female_financial_consultant_ff395274.png',
        'id' => 5
    ];
}
...
if (!empty($_POST['action'])) {
    if ($_POST['action'] === 'purchase' && $can_afford && !$already_purchased) {
        ...
        // Create notification (Firebase format)
                        ...
                        addUserNotification($_SESSION['email'], $notification);
